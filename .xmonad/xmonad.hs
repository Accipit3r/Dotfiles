...
modMask' = mod4Mask
-- Define workspaces
myWorkspaces    = clickable . (map dzenEscape) $ ["1:main","2:web","3:edit","4:chat","5:media", "6:manage"]
 
		where clickable l  	= [ "^ca(1,xdotool key super+" ++ show (n) ++ ")" ++ ws ++ "^ca()" |
                            (i,ws) <- zip [1..] l,
                            let n = i ]

--["1:main","2:web","3:edit","4:chat","5:media", "6:manage"]
-- Dzen/Conky
myXmonadBar = "dzen2 -x '0' -y '0' -h '24' -w '600' -ta 'l' -fg '#FFFFFF' -bg '#1B1D1E'"
myStatusBar = "conky -c /home/dennis/.xmonad/.conky_dzen | dzen2 -x '600' -w '767' -h '24' -ta 'r' -bg '#1B1D1E' -fg '#FFFFFF' -y '0'"
myBitmapsDir = "/home/dennis/xmonad/dzen2"
--}}}
-- Main {{{
main = do
    dzenLeftBar <- spawnPipe myXmonadBar
    dzenRightBar <- spawnPipe myStatusBar
    spawn "bash ~/.xmonad/start.sh"
...
manageHook' = (composeAll . concat $
    ...
    , [className    =? c            --> doShift  (myWorkspaces !! 0) |   c   <- myDev    ] -- move dev to main
    , [className    =? c            --> doShift  (myWorkspaces !! 1) |   c   <- myWebs   ] -- move webs to main
    , [className    =? c            --> doShift  (myWorkspaces !! 2) |   c   <- myVim    ] -- move webs to main
    , [className    =? c            --> doShift	 (myWorkspaces !! 3) |   c   <- myChat   ] -- move chat to chat
    , [className    =? c            --> doShift  (myWorkspaces !! 4) |   c   <- myMusic  ] -- move music to music
    , [className    =? c            --> doShift  (myWorkspaces !! 5) |   c   <- myManage ]-- move Thunderbird to manage
    ...
    where
        ...
        myMovie   = ["Boxee","Trine"]
        myMusic	  = ["Rhythmbox","Spotify","VLC","Vlc","vlc","clementine","Clementine"]
        myChat	  = ["Pidgin","Buddy List", "Psi", "Psi+", "chat", "psi","skype","Skype"]
        myGimp	  = ["Gimp"]
        myDev	  = ["gnome-terminal"]
        myVim	  = ["Gvim"]
        myManage  = ["thundrbird-bin","gecko","Thunderbird-bin","thunderbird","Thunderbird"]
...
-- }}}
layoutHook'  =  onWorkspaces [(myWorkspaces !! 0),(myWorkspaces !! 4)] customLayout $ 
                 
                onWorkspaces [(myWorkspaces !! 3)] imLayout $
                customLayout2
